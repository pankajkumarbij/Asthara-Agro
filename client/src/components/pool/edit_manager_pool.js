...
export default function EditManagerPool(props,{ navigation }) {
    ...
    function submitForm() {
        fetch(`http://localhost:5000/update_manager_pool/${id}`, {
            method: 'PUT',
            headers: {
        ...
        .then(data => {
            if(data.message!="Something wrong!"){
                ...
                if(data.err.errors){
                    ...
                    alert("Pin Code "+data.err.keyValue.postal_code+" is already available in another pool!");
                }
                else if(data.err.keyPattern.pool_name){
                    alert("Pool "+data.err.keyValue.pool_name+" is already created!");
                }
            }
        });
    }
        ...
        <ScrollView>
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
                <Card style={styles.card} >
                    <Card.Title title="Edit Manager Pool"/>
                    <Card.Content>
