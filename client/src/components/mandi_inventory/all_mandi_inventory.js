...
export default function AllMandiInventory({navigation }) {
    ...
    function scrap(item){
        ...
        fetch(`http://localhost:5000/delete_mandi_inventory_by_id/${item._id}`, {
            method: 'GET',
        })
        .then(res => res.json())
        .catch(error => console.log(error))
        ...
        <ScrollView>
            <View style={styles.view}>
                <DataTable style={styles.datatable}>
                    <Title style={styles.title} >Mandi Inventory</Title>
                    <Searchbar
