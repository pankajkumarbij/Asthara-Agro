...
export default function AllItemGrades(props,{ navigation }) {
    ...
    const sorting = (col)=>{
        if(sorting_order=="ASC"){
            const sorted=([...allItemGrade].sort((a,b)=>
            a[col].toLowerCase()>b[col].toLowerCase() ?1:-1));
            setAllItemGrade(sorted);
            setSortingOrder('DES');
        }
        ...
        <ScrollView>
            <View style={styles.view}>
                <DataTable style={styles.datatable}>
                    <Title style={styles.title} >All Item Grades</Title>
                    <Searchbar
                    ...
                    <DataTable.Header>
                        <DataTable.Title onPress={()=>sorting("grade_name")}><FontAwesomeIcon icon={ faSort } />Item Grade</DataTable.Title>
                            ...
                            return (
                                <DataTable.Row>
                                    ...
                                    <DataTable.Cell numeric>
                                        {Platform.OS=='android' ?
                                            <Button mode="contained" style={{width: '100%'}} onPress={() => {navigation.navigate('EditItemGrade', {itemGradeId: item._id})}}>Details</Button>
                                            :
                                            <Button mode="contained" style={{width: '100%'}}><Link to={"/edititemgrade/"+item._id}>Details</Link></Button>
                                        }
                                    </DataTable.Cell>
                                </DataTable.Row>
                            )
